...
namespace Utils
{
    ...
    void PwaHelper::InitAumidToAppId()
    {
        ...
        for (const auto subProcessID : pwaHelperProcessIds)
        {
            ...
            m_pwaAmuidToAppId.insert(std::map<std::wstring, std::wstring>::value_type(aumidID, appId));
    ...
    {
        const auto pwaIndex = m_pwaAmuidToAppId.find(windowAumid);
        if (pwaIndex != m_pwaAmuidToAppId.end())
        {
    ...
    std::wstring PwaHelper::SearchPwaName(const std::wstring& windowAumid, const std::wstring& windowAumid) const
    {
